...
class GitHubService {
	...
	private forEachDayInRange(
		start: Date,
		end: Date,
	callback: (currentDate: Date, dayIndex: number) => void
	): number {
	const normalizedStart = this.getStartOfDay(start)
	const normalizedEnd = this.getStartOfDay(end)
	...
	for (
			...
			callback(new Date(currentDate), dayIndex)
			dayIndex++
		}
	...
	if (shouldSkipLiveGitHubFetches()) {
      ...
      `
			...
			if (calendar.totalContributions === 0) {
				console.warn(
					'GitHub Graph: 0 contributions returned. Check username and token scopes.'
				)
			} else {
			}
	...
	year: number = new Date().getFullYear()
	): Promise<Map<string, GitHubContributionDay>> {
	...
	this.forEachDayInRange(startDate, endDate, currentDate => {
	...
	const totalDays = this.forEachDayInRange(start, end, currentDate => {
			const dateStr = currentDate.toISOString().split('T')[0]
			const dayData = dailyContributions.get(dateStr)
	...
	async getUserRepositories(
		sort: 'updated' | 'created' | 'pushed' | 'stars' = 'updated'
	): Promise<GitHubRepo[]> {
		try {
			const response = await fetch(
	...
	async getDetailedEvents(
		startDate: string,
		endDate: string
	): Promise<GitHubDayActivity[]> {
		try {
			const allEvents: any[] = []
	...
	switch (event.type) {
			...
			case 'PullRequestReviewEvent': {
				...
				const reviewBody =
					event.payload.review?.body?.split('\n')[0]?.trim() || ''
			...
			case 'PullRequestReviewCommentEvent': {
				...
				const commentBody =
					event.payload.comment?.body?.split('\n')[0]?.trim() || ''
				const filePath =
					event.payload.comment?.path?.split('/').pop() || ''
			...
			case 'GollumEvent': {
				...
				const action =
					pages[0]?.action === 'created' ? 'created' : 'edited'
			...
			case 'MemberEvent': {
				...
				const action =
					event.payload.action === 'added' ? 'added' : 'removed'
				return {
					...base,
					type: 'unknown',
