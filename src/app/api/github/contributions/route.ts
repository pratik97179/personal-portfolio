...
async function fetchWithRetry(year: number, maxRetries = 3) {
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			const contributions =
				await githubService.getDailyContributions(year)
...
export async function GET(request: Request) {
	try {
		...
		const contributionsArray = Array.from(contributions.entries()).map(
			([date, data]) => ({
				date,
				data
			})
		...
		console.error(
			'Error in /api/github/contributions after retries:',
			error
		)
		return NextResponse.json(
