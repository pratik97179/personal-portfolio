...
const LATEST_PROJECTS = [
	{
		...
		repo: process.env.NEXT_PUBLIC_PORTFOLIO_REPO?.trim() || githubUsername,
		name: 'portfolio',
...
function getHeaders(): Record<string, string> {
	const headers: Record<string, string> = {
		...
		'User-Agent': 'pratik-portfolio-activity'
	}
...
async function fetchGitHub<T>(endpoint: string): Promise<T> {
	...
	if (!response.ok) {
		...
		console.error(
			`GitHub API error: ${response.status} ${response.statusText}. Rate limit: ${rateLimitRemaining}`
		)
...
async function fetchGitHubActivity(limit = 30): Promise<GitHubActivity> {
	const events = await fetchGitHub<GitHubEvent[]>(
		`/users/${GITHUB_USERNAME}/events?per_page=${limit}`
	)
	return {
		events,
...
async function fetchLatestCommitForRepo(
	owner: string,
	repo: string
