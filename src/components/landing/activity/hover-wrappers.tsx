...
export function ProjectHoverWrapper({
	children,
	repository,
	isPrivate
}: WrapperProps) {
...
export function ActivityHoverWrapper({ children, activity }: HoverProps) {
	return (
		<ActivityHoverCard trigger={children} side="bottom" align="center">
			<GitHubActivityCard
				type={activity.type}
...
export function SpotifyHoverWrapper({ children, track, isPlaying }: Props) {
	return (
		<ActivityHoverCard trigger={children} side="bottom" align="end">
			<SpotifyCard
				name={track.name}
				artist={track.artist}
				album={track.album}
