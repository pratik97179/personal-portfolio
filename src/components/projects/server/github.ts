...
function getCachedMetrics(githubUrl: string): IGitMetrics | null | undefined {
	...
	if (cached.expiresAt <= wallClockNow()) {
...
function setCachedMetrics(
	...
	metricsCache.set(githubUrl, {
		value,
		expiresAt: wallClockNow() + ttlMs
	})
}

function getRateLimitCooldown(response: Response): number {
	const resetHeader = response.headers.get('x-ratelimit-reset')
	if (!resetHeader) return wallClockNow() + FAILURE_TTL_MS

	const resetAt = Number(resetHeader) * 1000
	return Number.isFinite(resetAt) ? resetAt : wallClockNow() + FAILURE_TTL_MS
}
...
async function safeFetch(
	...
	if (rateLimitResetAt > wallClockNow()) {
		return null
	}
