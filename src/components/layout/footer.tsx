...
import { Check, Copy, GitCommit, Github, Linkedin } from 'lucide-react'
...
const portfolioRepo =
	process.env.NEXT_PUBLIC_PORTFOLIO_REPO?.trim() || githubUsername
...
export function Footer() {
	...
	const relativeTimeInfo = useMemo(() => {
		...
		if (diffMins < 60)
			return { value: diffMins, unit: diffMins === 1 ? 'min' : 'mins' }
		if (diffHours < 24)
	...
	const siteLabel = profile.name.split(' ')[0].toLowerCase()
							...
							>
								{copied ? (
									...
									<>
										...
										<span>{profile.emailDisplay}</span>
									</>
								)}
							</button>
						</div>
